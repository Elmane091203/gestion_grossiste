...
<div class="card" style="box-shadow:2px 2px 20px blue,2px 2px 20px grey ;">
        <div class="card-header" style="background: rgb(2,36,36); color:white;text-align:center;">Modification Produit</div>
            <div class="card-body">
            <form action="{{route('produit.update',$produit->id)}}" enctype="multipart/form-data" method="POST">
                @csrf
                <label for="">Image</label><br>
                <img src="{{ url('public/images/'.$produit->image) }}" width="50" height="50" value="{{$produit->image}}">
